...
OUTPUT_DIR = PROJECT_ROOT / "Mapping" / "20x_3y" / "lightgbm"
...
def save_feature_importance(model: lgb.Booster, target: str) -> None:
    importance = model.feature_importance(importance_type="gain")
    df_imp = pd.DataFrame({"feature": FEATURE_COLUMNS, "importance_gain": importance})
...
def main() -> None:
    ...
    for target in TARGET_COLUMNS:
        ...
        model.save_model(str(OUTPUT_DIR / f"lgbm_{target}.txt"))
        save_feature_importance(model, target)
